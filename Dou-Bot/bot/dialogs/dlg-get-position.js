        ...
        if(nameEntity!=null){
            ...
            if(prvUserName==undefined && proNoun.indexOf(curUserName)!= -1 )
            {
                ...
                if(util.isArrayEmpty(profiles)){
                    session.send("Sorry we don't have any information about the position of "+curUserName);
                    session.endDialog();            
                }else{
                    sendPosition(session,profiles);
                }
                
            }else if(prvUserName!=undefined && proNoun.indexOf(curUserName)!= -1 ){
                let profiles=util.getProfiles(prvUserName);
                if(util.isArrayEmpty(profiles)){
                    session.send("Sorry we don't have any information about the position of "+prvUserName);
                    session.endDialog();            
                }else{
...
function createThumbnailCard(session,profile) {
    ...
    return new builder.ThumbnailCard(session)
        ...
        .subtitle('*Full Name: *'+profile.fullName+" *Position:* "+profile.position)
        .images([
...
function createHeroCard(session,profile) {
    ...
    return new builder.HeroCard(session)
        ...
        .title(profile.fullName)
        .subtitle("Position: "+profile.position);
}

function sendPosition(session, profiles) {
    if (profiles.length>0){
        sendPositionInformation(session,profiles);
    }
}


function sendPositionInformation(session, profiles) {
